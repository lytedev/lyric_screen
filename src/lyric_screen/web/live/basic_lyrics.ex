defmodule LyricScreen.Web.Live.BasicLyrics do
  ...
  defp do_set_slide_contents(
         %{
           assigns: %{
             display:
               %{frozen?: true, frozen_song: song_index, frozen_slide: slide_index} = display
           }
         } = socket
       ) do
  ...
  defp maybe_transition(
         %{assigns: %{current_slide_contents: cur, previous_slide_contents: prev}} = socket
       ) do
    if cur == prev do
      socket
    else
